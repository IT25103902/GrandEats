...
public class UserService {
    ...
    public User getUserByEmail(String email) {
        ...
        try (Connection con = DBConnection.getConnection();
            ...
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                return new User(rs.getString("userId"), rs.getString("fullName"), rs.getString("email"),
    ...
    public List<User> getAllUsers() {
        ...
        try (Connection con = DBConnection.getConnection();
            ...
            while (rs.next()) {
                userList.add(new User(rs.getString("userId"), rs.getString("fullName"), rs.getString("email"),
                        rs.getString("phoneNumber"), rs.getString("password"), rs.getString("role")));
            }
        } catch (SQLException e) { e.printStackTrace(); }
        return userList;
    ...
    public String getNextUserId() {
        // Find the largest user ID currently saved in the database
        ...
        try (Connection con = DBConnection.getConnection();
            ...
            if (rs.next()) {
                String lastId = rs.getString("userId"); // ex: U-10000
                if (lastId != null && lastId.length() > 2) {
                    int nextNum = Integer.parseInt(lastId.substring(2)) + 1; // ex: 10000 + 1 = 10001
                    return "U-" + nextNum;
                }
            }
        } catch (Exception e) {
        ...
        // If there are no users in the database yet, start with this first ID
        return "U-10000";
    }
}